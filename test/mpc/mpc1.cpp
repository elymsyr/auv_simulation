...
class NonlinearMPC {
    ...
    private:
        ...
        void setup_optimization() {
            ...
            if (!std::ifstream("libdynamics_func1.so")) {
                std::cout << "Generating libdynamics_func1.so...";
                ...
                CodeGenerator cg("libdynamics_func1", cg_options);
                cg.add(dynamics_func);
                cg.add(jac_dynamics);
                cg.add(grad_dynamics);
                cg.generate();

                std::system ("gcc -fPIC -shared -O3 libdynamics_func1.c -o libdynamics_func1.so");
            }
            dynamics_func = external("dynamics_func", "./libdynamics_func1.so", external_options);
