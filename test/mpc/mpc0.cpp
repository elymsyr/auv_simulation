...
public:
    ...
    MX damping_matrix(const MX& nu) const {
        MX Dn = mtimes(Dn_, MX::diag(fabs(nu)));
        return Dn + Dl_;
    }
    ...
    std::pair<MX, MX> dynamics(const MX& eta, const MX& nu, const MX& tau_p) const {
        ...
        MX nu_dot = mtimes(M_inv_, tau - mtimes(C, nu) - mtimes(D, nu) - g);
...
private:
    ...
    void calculate_linear() {
        ...
        MX A11 = MX::diag(MX::vertcat({MX(C_X_), MX(C_Y_), MX(C_Z_)}));
        MX A12 = MX::zeros(3,3);
        A12(1,2) = C_Z_q_;
        A12(2,1) = C_Y_r_;
        MX A21 = A12.T();
...
class NonlinearMPC {
    public:
        ...
        std::pair<DM, DM> solve(const DM& x0, const DM& x_ref) {
            ...
            if (prev_sol_.has_value()) {
                ...
                // Warm-start: shift previous solution one time step forward.
                DM X_guess = horzcat(prev_X(Slice(), Slice(1, N_+1)), prev_X(Slice(), N_));
                DM U_guess = horzcat(prev_U(Slice(), Slice(1, N_)), prev_U(Slice(), N_-1));
    ...
    private:
        ...
        void setup_optimization() {
            ...
            MX xdot = MX::vertcat({dyn_pair.first, dyn_pair.second});
            
            Dict external_options = {{"jit", true}, {"compiler", "shell"}, {"jit_cleanup", true}};

            Function dynamics_func = casadi::Function("dynamics_func", {eta_sym, nu_sym, u_sym}, {xdot}, external_options);

            if (!std::ifstream("libdynamics_func0.so")) {
                std::cout << "Generating libdynamics_func0.so...";
                ...
                CodeGenerator cg("libdynamics_func0", cg_options);
                cg.add(dynamics_func);
                cg.add(jac_dynamics);
                cg.add(grad_dynamics);
                cg.generate();

                std::system ("gcc -fPIC -shared -O3 libdynamics_func0.c -o libdynamics_func0.so");
            }
            dynamics_func = external("dynamics_func", "./libdynamics_func0.so", external_options);
